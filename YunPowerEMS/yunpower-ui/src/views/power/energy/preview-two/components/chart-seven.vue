<template>
  <a-card :title="$t('power.energy.preview.powerFactor')">
    <template #extra>
...
const timeChange = async (options: any) => {
  ...
  if (options.timeUnit == 0) {
    await fetchData({ pageValue: PowerModuleEunm.power_factor_day, "dayTime": options.timer });
  } else if (options.timeUnit == 1) {
    await fetchData({ pageValue: PowerModuleEunm.power_factor_month, "monthTime": options.timer });
  } else {
    await fetchData({ pageValue: PowerModuleEunm.power_factor_year, "yearTime": options.timer });
  }
}
...
const fetchData = async (param: any) => {
  ...
  try {
    ...
    if (res.code == 200) {
      ...
      console.log("平均功率因素监控", param.pageValue, chartData.value)
    }
  } catch (ex) {
    console.error("平均功率因素监控出错", ex)
  } finally {
    setLoading(false);
  }
}
